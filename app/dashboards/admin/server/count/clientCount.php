...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as client FROM user WHERE userType = 'client'");
    $stmt->execute();
    $result = $stmt->fetch();
    
    if ($result !== false && isset($result['client'])) {
        echo json_encode(['count' => $result['client']]);
    } else {
    ...
    error_log("Database error: " . $e->getMessage() . " | Query: SELECT COUNT(*) as client FROM user WHERE userType != 'admin'");
