...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as users FROM user WHERE userType != 'admin'");
    $stmt->execute();
    $result = $stmt->fetch();
    
    if ($result !== false && isset($result['users'])) {
        echo json_encode(['count' => $result['users']]);
    } else {
    ...
    error_log("Database error: " . $e->getMessage() . " | Query: SELECT COUNT(*) as users FROM user WHERE userType != 'admin'");
